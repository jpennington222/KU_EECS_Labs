...
char* fmrCompute(fmrNode* node){
	...
	if(pid_1 == 0){	//step 3 - condition for first child	
	...
	if(pid_2 == 0){   /*STEP 7 - Condition for second child*/
	...
	if(pid_3 == 0){   /*STEP 11 - Condition for third child*/
	...
	if(pid_4 == 0){   /*STEP 15 - Condition for fourth child*/
...
//		if(pid_fmr == 0){  /*EXTRA CREDIT - STEP 3 - Check condition for child process*/
...
//	while(read(p0[0],val,sizeof(val))){   /*EXTRA CREDIT - STEP 8 - read from p0 and store into val*/
